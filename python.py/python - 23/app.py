# # # 23-dars: MODULLAR


import avto_info_mod

avto1 = avto_info_mod.avto_info("GM", "Malibu", "Qora", "avtomat", 2020,40000)
avto_info_mod.info_print(avto1)

import avto_info_mod as aim

avto1 = aim.avto_info("GM", "Malibu", "Qora", "avtomat", 2020,40000)
aim.info_print(avto1)

from avto_info_mod import avto_info, info_print

avto1 = avto_info("GM", "Malibu", "Qora", "avtomat", 2020,40000)
info_print(avto1)

from avto_info_mod import avto_info as ainfo, info_print as iprint

avto1 = ainfo("GM", "Malibu", "Qora", "avtomat", 2020,40000)
iprint(avto1)

from avto_info_mod import *

avto1 = avto_info("GM", "Malibu", "Qora", "avtomat", 2020,40000)
info_print(avto1)

avtolar = avtoil.avto_kirit()

for avto in avtolar:
     avtoil.info_print(avto)
     
#

import math 

x=400
print(math.sqrt(x)) # sqrt(x) - kirtilgan qiymatni kvadrat ildizini xisoblaydi
print(math.pow(6,2)) # pow(6,2) - power sonning malum bir darajasi
print(math.pi) # 
print(math.log2(8))
print(math.log10(100))
     
#
import random as r# random tasodifiy sonlarni generatsiya qiladi


# # randint() tasodifiy soni chiqarb berad      
son = r.randint(10,50)
print(son)

# # choice() - sizdagi biror bir ro'yxatni ichidan tasodifiy qiymatni tanlab oladi
ismlar = ['olim','anvar','hasan','husan']
ism = r.choice(ismlar)
print(ism)
print(r.choice(ism))
x = list(range(0,51,5))
print(x)
print(r.choice(x))

# shuffle() - aralashtiriib tashlash , qiymatlarni o'rnini almashtirib tashlaydi

x = list(range(11))
print(x)
r.shuffle(x)
print(x)